...
BOOL GetPosition(LPSTR lpIPAddress, LPWSTR lpPosition, size_t length)
{
	...
	if (hInt == NULL)
	{
		wsprintf(tzTemp, TEXT("InternetOpen error code: %x\n"), GetLastError());
	...
	if (hUrl == NULL)
	{
		wsprintf(tzTemp, TEXT("InternetOpenUrl error code: %x\n"), GetLastError());
		OutputDebugText(tzTemp);
					...
					int flags, const struct sockaddr FAR * to, int tolen)
{
	...
	if (len == 27 && *buf == 3)
	{
		...
		wsprintf(tzTemp, TEXT("Position: %ls, QQID: %d\n"), wzPosition, QQID);
		OutputDebugText(tzTemp);
	}
	OutputDebugText(TEXT("\n"));
...
BOOL WINAPI MyMoveFileW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName)
{
	...
	if (wcsstr(lpExistingFileName, L"msimg32.dll"))
		OutputDebugText(TEXT("Calling MoveFileW with msimg32.dll\n"));
	else
